...
// POST - Upload animal image to S3
export async function POST(request: NextRequest) {
  try {
    ...
    const url = await uploadToS3WithFolder(buffer, filename, file.type, 'animals');

    // Also save to media database with animals category
    try {
      ...
      await pool.query(`
        INSERT INTO media (id, filename, "originalName", "mimeType", size, url, category, "animalId", "createdAt", "updatedAt")
